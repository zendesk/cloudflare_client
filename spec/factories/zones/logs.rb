FactoryGirl.define do
  factory :zone_logs, class: Hash do
    ...
    factory :zone_log do
      brandId { Faker::Number.number(3).to_i }
      flags { Faker::Number.number(1).to_i }
      hosterId { Faker::Number.number(1).to_i }
      ownerId { Faker::Number.number(7).to_i }
      rayId '36527575c4556dcc'
      securityLevel 'high'
      timestamp { Time.now.utc.iso8601(9) }
      unstable nil
      zoneId { Faker::Number.number(8).to_i }
    ...
    factory :zone_log_client do
      asNum { Faker::Number.number(5).to_i }
      country 'us'
      deviceType 'desktop'
      ip { Faker::Internet.ip_v4_address }
      ipClass 'noRecord'
      srcPort { Faker::Number.number(4).to_i }
      sslCipher 'NONE'
      sslFlags 0
      sslProtocol 'none'
    end
    ...
    factory :zone_log_client_request do
      ...
      body nil
      bodyBytes 0
      bytes { Faker::Number.number(2).to_i }
      cookies nil
      dnt 'unset'
      flags 0
      headers []
    ...
    factory :zone_log_edge_rate_limit do
      ...
      ruleId { Faker::Number.number(5).to_i }
    ...
    factory :zone_log_edge_response do
      bodyBytes { Faker::Number.number(4).to_i }
      bytes { Faker::Number.number(4).to_i }
      compressionRatio 0
